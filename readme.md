...
我们在单机环境下对 Mini-Redis 进行了严格的压力测试，以验证 **"Netty I/O 多线程 + 业务单线程无锁"** 架构的实际性能表现。

### 测试环境
-   **硬件**: 普通开发笔记本 (具体配置取决于您的机器)
-   **JVM**: JDK 17 (G1GC)
-   **工具**: 自研 Netty 压测客户端 (`MiniRedisBenchmark`)
-   **策略**: 50 并发连接，Pipeline 流水线模式，3 轮预热 + 5 轮正式测试取平均值。

### 测试结果 (QPS)

| 命令类型 | 指令 | 平均 QPS | 备注 |
| :--- | :--- | :--- | :--- |
| **String** | `SET` | **95,900** | 纯内存写入，无锁竞争 |
| | `GET` | **189,550** | 纯内存读取，极低延迟 |
| **Hash** | `HSET` | **98,554** | `HashMap` 写入 |
| | `HGET` | **189,198** | O(1) 快速查询 |
| **List** | `LPUSH` | **97,044** | `QuickList` 头部插入 |
| | `LRANGE` | **139,553** | 遍历 `QuickList` (Range 0-10) |
| **Set** | `SADD` | **95,457** | `HashSet` 写入 |
| | `SISMEMBER` | **98,169** | O(1) 存在性检查 |
| **ZSet** | `ZADD` | **90,016** | `SkipList` O(logN) 插入 |
| | `ZRANGE` | **233,896** | `SkipList` 遍历 |

### 性能分析
1.  **写操作 (Write)**: 平均稳定在 **9.5万 QPS** 左右。得益于单线程无锁架构，消除了 `synchronized` 上下文切换开销，吞吐量主要受限于对象分配与 GC。
2.  **读操作 (Read)**: 平均稳定在 **19万+ QPS**。读取路径无内存分配开销，性能主要受限于 Netty 网络 I/O 处理能力。
3.  **对比**: 相比于未优化前的多线程锁竞争版本，**整体吞吐量提升了 5~6 倍**。这验证了在 IO 密集型与内存操作密集型混合场景下，Reactor 模式配合单线程业务处理的优越性。
